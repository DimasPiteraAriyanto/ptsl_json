...
<html>
<head>
	<title>Entri Persetujuan Keluarga</title>
...
<body style="background-color: #FFF" onblur="self.focus();">
	      ...
	      <div class="card-box table-responsive">
	        ...
	        <table id="datatable_responsive" class="table table-striped responsive nowrap" cellspacing="0" width="100%">
	          <thead>
	            <tr>
	              <th>ID</th>
	              <th>Nama Persetujuan Keluarga</th>
	              <th>NIK</th>
	              <th>No. Telp</th>
	              <th>Alamat</th>
	              <th>Proses</th>
	...
	@include('auths.persetujuan-keluarga.form')
</body>
<!-- jQuery -->
...
<script type="text/javascript">
  	...
  	function deleteRecord(id) {
  		...
  		if(result == true){
			...
			$.ajax({
	            type: 'DELETE',
	            url: SITEURL + 'persetujuan-keluarga/' + id,
	...
	function inputRecord(id) {
  		...
  		if(result == true){
			...
			$.ajax({
	            type: 'POST',
	            url: SITEURL + 'entri-persetujuan-keluarga',
	            data: {
                    'id' : id,
                    'session' : $('#session').val(),
                },
	...
	function editRecord(id) {
  		$(document).on('click', '.btn-edit', function() {
  			$('.modal-title').text('Form Ubah Persetujuan Keluarga')
        	$('.f-add').modal('show');
        	$('.add').hide();
            $('.update').show();
    	});
    	...
    	$.ajax({
            type: 'GET',
            url: SITEURL + "json/persetujuan-keluarga/" + id,
            ...
            success: function(data) {
	        	...
	        	$('#nama_persetujuan_keluarga').val(data.nama_persetujuan_keluarga)
            	...
            	var clickHandler = function(e){
			        $.ajax({
			            ...
			            data: {
		                    'nik' : $('#nik').val(),
		                    'no_telp' : $('#no_telp').val(),
		                    'nama_persetujuan_keluarga' : $('#nama_persetujuan_keluarga').val(),
	...
	$(document).ready( function () {
		...
		var tableData = $('#datatable_responsive').DataTable({
			...
			ajax: {
				type: 'POST',
				url: SITEURL + "json/persetujuan-keluarga",
			},
		...
		$(document).on('click', '.m-add', function() {
            ...
            $('#form-add')[0].reset();
            $('.f-add').modal('hide');
  			$('.modal-title').text('Form Tambah Persetujuan Keluarga')
        	$('.f-add').modal('show');
        	$('.add').show();
            $('.update').hide();
    	});
	    ...
	    $('.modal-footer').on('click', '.add', function() {
	        ...
	        var nama_persetujuan_keluarga = $('#nama_persetujuan_keluarga').val();
	        ...
	        data.append('nama_persetujuan_keluarga', nama_persetujuan_keluarga);
	        ...
	        $.ajax({
	            type: 'POST',
	            url: SITEURL + "persetujuan-keluarga",
	            cache       : false,
	            contentType : false,
	            processData : false,
	            data: data,
