...
<html>
<head>
	<title>Entri Pihak Pertama</title>
...
<body style="background-color: #FFF">
	...
	@include('auths.pihak-pertama.form')
</body>
<!-- jQuery -->
...
<script type="text/javascript">
	...
	function inputRecord(id) {
  		...
  		if(result == true){
			...
			$.ajax({
	            type: 'POST',
	            url: SITEURL + 'entri-pihak-pertama',
	            data: {
                    'id' : id,
                    'session' : $('#session').val(),
                },
	...
	function editRecord(id) {
  		$(document).on('click', '.btn-edit', function() {
  			$('.modal-title').text('Form Ubah Pihak Pertama')
        	$('.f-add').modal('show');
        	$('.add').hide();
            $('.update').show();
    	});
    	...
    	$.ajax({
            type: 'GET',
            url: SITEURL + "json/pihak-pertama/" + id,
            ...
            success: function(data) {
	        	...
	        	$('#nama_pihak_pertama').val(data.nama_pihak_pertama)
            	...
            	var clickHandler = function(e){
			        $.ajax({
			            ...
			            data: {
		                    'nik' : $('#nik').val(),
		                    'no_telp' : $('#no_telp').val(),
		                    'nama_pihak_pertama' : $('#nama_pihak_pertama').val(),
	...
	$(document).ready( function () {
		...
		var tableData = $('#datatable_responsive').DataTable({
			...
			ajax: {
				type: 'POST',
				url: SITEURL + "json/pihak-pertama",
			},
		...
		$(document).on('click', '.m-add', function() {
            ...
            $('#form-add')[0].reset();
            $('.f-add').modal('hide');
  			$('.modal-title').text('Form Tambah Pihak Pertama')
        	$('.f-add').modal('show');
        	$('.add').show();
            $('.update').hide();
    	});
	    ...
	    $('.modal-footer').on('click', '.add', function() {
	        ...
	        var nama_pihak_pertama = $('#nama_pihak_pertama').val();
	        ...
	        data.append('nama_pihak_pertama', nama_pihak_pertama);
