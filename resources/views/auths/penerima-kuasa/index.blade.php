...
<html>
<head>
	<title>Entri Penerima Kuasa</title>
...
<body style="background-color: #FFF">
	...
	@include('auths.penerima-kuasa.form')
</body>
<!-- jQuery -->
...
<script type="text/javascript">
	...
	function inputRecord(id) {
  		...
  		if(result == true){
			...
			$.ajax({
	            type: 'POST',
	            url: SITEURL + 'entri-penerima-kuasa',
	            data: {
                    'id' : id,
                    'session' : $('#session').val(),
                },
	...
	function editRecord(id) {
  		$(document).on('click', '.btn-edit', function() {
  			$('.modal-title').text('Form Ubah Penerima Kuasa')
        	$('.f-add').modal('show');
        	$('.add').hide();
            $('.update').show();
    	});
    	...
    	$.ajax({
            type: 'GET',
            url: SITEURL + "json/penerima-kuasa/" + id,
            ...
            success: function(data) {
	        	...
	        	$('#nama_penerima_kuasa').val(data.nama_penerima_kuasa)
            	...
            	var clickHandler = function(e){
			        $.ajax({
			            ...
			            data: {
		                    'nik' : $('#nik').val(),
		                    'no_telp' : $('#no_telp').val(),
		                    'nama_penerima_kuasa' : $('#nama_penerima_kuasa').val(),
	...
	$(document).ready( function () {
		...
		var tableData = $('#datatable_responsive').DataTable({
			...
			ajax: {
				type: 'POST',
				url: SITEURL + "json/penerima-kuasa",
			},
		...
		$(document).on('click', '.m-add', function() {
            ...
            $('#form-add')[0].reset();
            $('.f-add').modal('hide');
  			$('.modal-title').text('Form Tambah Penerima Kuasa')
        	$('.f-add').modal('show');
        	$('.add').show();
            $('.update').hide();
    	});
	    ...
	    $('.modal-footer').on('click', '.add', function() {
	        ...
	        var nama_penerima_kuasa = $('#nama_penerima_kuasa').val();
	        ...
	        data.append('nama_penerima_kuasa', nama_penerima_kuasa);
